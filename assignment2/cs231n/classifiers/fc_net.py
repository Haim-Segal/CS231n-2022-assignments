...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        p = self.dropout_param['p'] if self.use_dropout else 1
        ...
        a = X.copy()
        for l in range(L - 1):
          args = (a, self.params['W' + str(l + 1)], self.params['b' + str(l + 1)])
          if self.normalization != None:
            args = (*args, self.params['gamma' + str(l + 1)],
        ...
        da, dW, grads['b' + str(L)] = affine_backward(dscores, caches[-1]) 
        grads['W' + str(L)] = dW + self.reg * self.params['W' + str(L)]
        if p < 1:
          da = dropout_backward(da, do_caches[-1])
        for l in reversed(range(L - 1)):
          if self.normalization == None:                        
            da, dW, grads['b' + str(l + 1)] = affine_relu_backward(da, caches[l])
          else:
            if self.normalization == "batchnorm":
              f = affine_bn_relu_backward
            else:
              f = affine_ln_relu_backward
            da, dW, grads['b' + str(l + 1)], grads['gamma' + str(l + 1)], grads['beta' + str(l + 1)] =  f(da, caches[l])
          grads['W' + str(l + 1)] = dW + self.reg * self.params['W' + str(l + 1)]
          if p < 1 and l > 0:
            da = dropout_backward(da, do_caches[l - 1])
